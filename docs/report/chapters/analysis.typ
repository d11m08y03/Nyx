...
FR2.4 TESS Observation Listing: The subsystem can return a list of observations of the astronomical target that has been looked up. This list will have one observation record for each observation of that target that is stored in the tess_observations} table. Each observation record will contain the obsid}, sequence}, target_name}, start_time}, end_time}, and the duration} of the observation (duration in seconds), as well as the lc_points} for that observation. The lc_points} value will be zero if the light curve for that observation has not yet been downloaded.

FR2.5 FITS Product Discovery: For a particular TESS observation, the subsystem can retrieve the list of FITS data products related to that observation. The list of related products can be obtained by making a request to the Mast.Caom.Products service of the MAST API. The response to this request will be a list of products related to the observation. The subsystem parses this list of products to find the FITS light curve file; this file will have a .lc.fits file extension. The dataURI of that FITS file is stored in the tess_observations table associated with the observation.

FR2.6 FITS Download and Parse: The FITS file can be downloaded from the MAST archive using the dataURI of the file. The file is parsed to extract the light curve data. The FITS file contains a binary table that stores the light curve data. The table has the following columns: the TIME of the observation, the PDCSAP_FLUX, SAP_FLUX of each pixel of the TESS detector, and the QUALITY of each flux measurement. The parser of the FITS file uses the TIME, PDCSAP_FLUX, \{SAP_FLUX, and QUALITY columns (@pence2010). The time values are stored as a double-precision floating-point number, the flux values are stored as double-precision floating point number, and the quality value of each pixel is stored as a 32-bit integer. Additionally, TESS light curves may contain NaN values for flux measurements; these values are ignored by the FITS parser (@tenenbaum2018). Furthermore, the FITS file is parsed using raw binary data and pointer arithmetic to access the relevant fields in the file; the CFITSIO library is avoided to minimize the number of external libraries required to compile Nyx.

FR2.7 Light Curve Storage: The light curve points that are parsed from the FITS file are stored in the database. Each record in the table light_curve_points contains the TIME of the observation as a double-precision floating point number, the PDCSAP_FLUX, SAP_FLUX values of the pixel in the detector, and the QUALITY of that flux measurement. The record also has a foreign key that links the point to the TESS observation from which the point was derived. An index on the \{tess_observation_id and \{time fields of the \{light_curve_points table enables fast retrieval of the points for a given observation. The points are inserted into the table in bulk to minimize the number of database round trips. Should the subsystem determine that there are already light curve points for that TESS observation in the database, those points are first deleted before the bulk-insertion of the newly parsed points.
