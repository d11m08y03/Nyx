#pragma once

#include "application/observation/ObservationService.hpp"

#include <drogon/HttpController.h>
#include <memory>

namespace Nyx::Presentation::Http::Observation {
  class ObservationSessionController
    : public drogon::HttpController<
        ObservationSessionController
      > {
    public:
      METHOD_LIST_BEGIN

      ADD_METHOD_TO(
        ObservationSessionController::create,
        "/api/v1/observation-sessions",
        drogon::Post,
        "Nyx::Presentation::Middleware::CorrelationIdFilter",
        "Nyx::Presentation::Middleware::JwtAuthFilter",
        "Nyx::Presentation::Middleware::CsrfFilter"
      );
      ADD_METHOD_TO(
        ObservationSessionController::list,
        "/api/v1/observation-sessions",
        drogon::Get,
        "Nyx::Presentation::Middleware::CorrelationIdFilter",
        "Nyx::Presentation::Middleware::JwtAuthFilter"
      );
      ADD_METHOD_TO(
        ObservationSessionController::get_by_id,
        "/api/v1/observation-sessions/{id}",
        drogon::Get,
        "Nyx::Presentation::Middleware::CorrelationIdFilter",
        "Nyx::Presentation::Middleware::JwtAuthFilter"
      );
      ADD_METHOD_TO(
        ObservationSessionController::update,
        "/api/v1/observation-sessions/{id}",
        drogon::Put,
        "Nyx::Presentation::Middleware::CorrelationIdFilter",
        "Nyx::Presentation::Middleware::JwtAuthFilter",
        "Nyx::Presentation::Middleware::CsrfFilter"
      );
      ADD_METHOD_TO(
        ObservationSessionController::remove,
        "/api/v1/observation-sessions/{id}",
        drogon::Delete,
        "Nyx::Presentation::Middleware::CorrelationIdFilter",
        "Nyx::Presentation::Middleware::JwtAuthFilter",
        "Nyx::Presentation::Middleware::CsrfFilter"
      );
      ADD_METHOD_TO(
        ObservationSessionController::run_photometry,
        "/api/v1/observation-sessions/{id}/photometry",
        drogon::Post,
        "Nyx::Presentation::Middleware::CorrelationIdFilter",
        "Nyx::Presentation::Middleware::JwtAuthFilter",
        "Nyx::Presentation::Middleware::CsrfFilter"
      );

      METHOD_LIST_END

      ObservationSessionController();

      auto create(
        const drogon::HttpRequestPtr& request,
        std::function<void(
          const drogon::HttpResponsePtr&
        )>&& callback
      ) -> void;

      auto list(
        const drogon::HttpRequestPtr& request,
        std::function<void(
          const drogon::HttpResponsePtr&
        )>&& callback
      ) -> void;

      auto get_by_id(
        const drogon::HttpRequestPtr& request,
        std::function<void(
          const drogon::HttpResponsePtr&
        )>&& callback,
        const std::string& id
      ) -> void;

      auto update(
        const drogon::HttpRequestPtr& request,
        std::function<void(
          const drogon::HttpResponsePtr&
        )>&& callback,
        const std::string& id
      ) -> void;

      auto remove(
        const drogon::HttpRequestPtr& request,
        std::function<void(
          const drogon::HttpResponsePtr&
        )>&& callback,
        const std::string& id
      ) -> void;

      auto run_photometry(
        const drogon::HttpRequestPtr& request,
        std::function<void(
          const drogon::HttpResponsePtr&
        )>&& callback,
        const std::string& id
      ) -> void;

    private:
      std::shared_ptr<
        Nyx::Application::Observation::ObservationService
      > observation_service;
  };
} // namespace Nyx::Presentation::Http::Observation
